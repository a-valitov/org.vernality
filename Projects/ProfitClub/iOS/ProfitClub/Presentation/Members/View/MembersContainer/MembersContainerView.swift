...
enum MembersContainerState {
    case membersOfOrganization
    case applications
}

protocol MembersContainerViewInput: UIViewController {
    var state: MembersContainerState { get set }
    var membersOfOrganization: UIViewController? { get set }
    var applications: UIViewController? { get set }
}

protocol MembersContainerViewOutput {
