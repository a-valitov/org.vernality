...
final class ReviewViewAlphaAdminCell: UITableViewCell {

    static let reuseIdentifier = "ReviewViewAlphaAdminCellReuseIdentifier"
    ...
    lazy var adminStatusLabel: UILabel = {
        let label = UILabel()
...
// MARK: - Layout
extension ReviewViewAlphaAdminCell {
    private func layout() {
        layoutAdminLabel(in: self.contentView)
        layoutAdminStatusLabel(in: self.contentView)
    }

    private func layoutAdminLabel(in container: UIView) {
        let label = self.adminLabel
    ...
    private func layoutAdminStatusLabel(in container: UIView) {
        let label = self.adminStatusLabel
        container.addSubview(label)
        NSLayoutConstraint.activate([
            label.topAnchor.constraint(equalTo: self.adminLabel.bottomAnchor, constant: 20.0),
