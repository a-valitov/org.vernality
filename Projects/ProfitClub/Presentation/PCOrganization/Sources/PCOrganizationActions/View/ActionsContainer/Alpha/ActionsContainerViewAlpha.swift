...
final class ActionsContainerViewAlpha: UIViewController {
        ...
        didSet {
            if isViewLoaded {
                if let vc = self.current {
                    self.embed(controller: vc, in: self.currentActionContainer)
                }
            }
        }
    }
    ...
    var past: UIViewController? {
        didSet {
            if isViewLoaded {
                if let vc = self.past {
                    self.embed(controller: vc, in: self.pastActionContainer)
                }
            }
        }
    }
    ...
    private lazy var segmentedControl: UISegmentedControl = {
        ...
        segmentedControl.addTarget(self, action: #selector(ActionsContainerViewAlpha.actionsSegmentedControlValueChanged(_:)), for: .valueChanged)
        ...
        segmentedControl.insertSegment(withTitle: "Прошедшие", at: 1, animated: true)
    ...
    private lazy var currentActionContainer: UIView = {
        let view = UIView()
    ...
    private lazy var pastActionContainer: UIView = {
        let view = UIView()
        view.backgroundColor = .white
        view.translatesAutoresizingMaskIntoConstraints = false
        return view
    ...
    private func updateState() {
        if isViewLoaded {
            ...
            case .current:
                ...
                self.currentActionContainer.isHidden = false
                self.pastActionContainer.isHidden = true
            case .past:
                self.segmentedControl.selectedSegmentIndex = 1
                self.currentActionContainer.isHidden = true
                self.pastActionContainer.isHidden = false
            }
        }
    }

    @objc func actionsSegmentedControlValueChanged(_ sender: UISegmentedControl) {
...
// MARK: - Layout
extension ActionsContainerViewAlpha {
    private func layout() {
        ...
        layoutPastActionsContainer(in: self.view)
        layoutCurrentActionsContainer(in: self.view)
    }
    ...
    private func layoutCurrentActionsContainer(in container: UIView) {
        let currentContainer = currentActionContainer
    ...
    private func layoutPastActionsContainer(in container: UIView) {
        let pastContainer = pastActionContainer
...
extension ActionsContainerViewAlpha: ActionsContainerViewInput {
