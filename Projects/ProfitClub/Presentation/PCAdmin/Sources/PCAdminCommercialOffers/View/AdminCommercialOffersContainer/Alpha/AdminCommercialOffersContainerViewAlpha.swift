...
final class AdminCommercialOffersContainerViewAlpha: UIViewController {
    var output: AdminCommercialOffersContainerViewOutput?
    var state: AdminCommercialOffersContainerState = .applications {
        didSet {
            updateState()
        }
    }
        ...
        didSet {
            if isViewLoaded {
                if let vc = self.approved {
                    self.embed(controller: vc, in: self.approvedCommercialOffersContainer)
                }
            }
        }
    }
    ...
    private lazy var segmentedControl: UISegmentedControl = {
        ...
        segmentedControl.addTarget(self, action: #selector(AdminCommercialOffersContainerViewAlpha.commercialOffersSegmentedControlValueChanged(_:)), for: .valueChanged)
    ...
    private lazy var approvedCommercialOffersContainer: UIView = {
        let view = UIView()
    ...
    override func viewDidLoad() {
        ...
        setup()
        layout()
        self.output?.adminCommercialOffersContainerDidLoad(view: self)
    }
    ...
    private func updateState() {
        if isViewLoaded {
            ...
            case .applications:
                ...
                self.approvedCommercialOffersContainer.isHidden = true
            case .approved:
                self.segmentedControl.selectedSegmentIndex = 1
                self.approvedCommercialOffersContainer.isHidden = false
                self.applicationsContainer.isHidden = true
            }
        }
    }

    @objc private func commercialOffersSegmentedControlValueChanged(_ sender: UISegmentedControl) {
        changeSegmentedControlLinePosition()
        switch sender.selectedSegmentIndex {
        case 0:
            self.output?.adminCommercialOffersContainer(view: self, didChangeState: .applications)
        case 1:
            self.output?.adminCommercialOffersContainer(view: self, didChangeState: .approved)
        default:
            break
        }
    }
...
// MARK: - Layout
extension AdminCommercialOffersContainerViewAlpha {
    private func layout() {
        ...
        layoutApprovedCommercialOffersContainer(in: self.view)
    ...
    private func layoutApprovedCommercialOffersContainer(in container: UIView) {
        let currentContainer = approvedCommercialOffersContainer
...
extension AdminCommercialOffersContainerViewAlpha: AdminCommercialOffersContainerViewInput {
