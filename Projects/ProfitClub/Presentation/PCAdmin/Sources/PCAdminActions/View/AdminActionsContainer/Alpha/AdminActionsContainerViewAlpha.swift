...
final class AdminActionsContainerViewAlpha: UIViewController {
    var output: AdminActionsContainerViewOutput?
    var state: AdminActionsContainerState = .applications {
        didSet {
            updateState()
        }
    }
        ...
        didSet {
            if isViewLoaded {
                if let vc = self.approved {
                    self.embed(controller: vc, in: self.approvedActionsContainer)
                }
            }
        }
    }
    ...
    private lazy var segmentedControl: UISegmentedControl = {
        ...
        segmentedControl.addTarget(self, action: #selector(AdminActionsContainerViewAlpha.actionsSegmentedControlValueChanged(_:)), for: .valueChanged)
    ...
    private lazy var approvedActionsContainer: UIView = {
        let view = UIView()
    ...
    override func viewDidLoad() {
        ...
        setup()
        layout()
        self.output?.adminActionsContainerDidLoad(view: self)
    }
    ...
    private func updateState() {
        if isViewLoaded {
            ...
            case .applications:
                ...
                self.approvedActionsContainer.isHidden = true
            case .approved:
                self.segmentedControl.selectedSegmentIndex = 1
                self.approvedActionsContainer.isHidden = false
                self.applicationsContainer.isHidden = true
            }
        }
    }

    @objc private func actionsSegmentedControlValueChanged(_ sender: UISegmentedControl) {
        changeSegmentedControlLinePosition()
        switch sender.selectedSegmentIndex {
        case 0:
            self.output?.adminActionsContainer(view: self, didChangeState: .applications)
        case 1:
            self.output?.adminActionsContainer(view: self, didChangeState: .approved)
        default:
            break
        }
    }
...
// MARK: - Layout
extension AdminActionsContainerViewAlpha {
    private func layout() {
        ...
        layoutActionsContainer(in: self.view)
    ...
    private func layoutActionsContainer(in container: UIView) {
        let currentContainer = approvedActionsContainer
...
extension AdminActionsContainerViewAlpha: AdminActionsContainerViewInput {
